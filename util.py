...
class Contact():
    def __init__(self, id, name=None):
        self.id = str(id)            # tne unique value (phone #, email, username, etc) that identifies this contact
        self.name = str(name)        # the name used for the contact
...
class Message():
    def __init__(self, sender, receiver, date_sent, content):
        ...
        self.date_sent = str(date_sent)      # expected in utc time
...
class Conversation():
    def __init__(self, person1, person2, messages=[]):
